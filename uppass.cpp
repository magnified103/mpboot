...
//            posU += stepU;
//            posPUp += stepPUp;
            posU += INTS_PER_VECTOR;
            posPUp += INTS_PER_VECTOR;
        }
        x = VECTOR_AND_NOT(x, allOne);
        // x = ~x;
        // posUUp = posPUp = 0;
...
//            posV1 += stepV1;
//            posV2 += stepV2;
            posV1 += INTS_PER_VECTOR;
            posV2 += INTS_PER_VECTOR;
        }
        t_N = VECTOR_AND_NOT(t_N, allOne);
        // t_N = ~t_N;
        int posU = 0;
 ...
 */
void dfsRecalculateUppass(nodeptr u, parsimonyNumber *pUpVec, pInfo *prModel, std::size_t w, std::size_t mxTips) {
    int width = prModel->parsimonyLength;
    int states = prModel->states;
//    int posU = width * states * u->number + w;
    std::size_t posU = table_index(width, states, u->number, 0, w);
...
//            &(prModel->parsVect[width * states * v2->number + w]),
        ...
        if (!equalStatesCmp(&(prModel->parsVectUppassLocal[posU]),
                            &(prModel->parsVectUppass[posU]), states)) {
            dfsRecalculateUppass(v1, &(prModel->parsVectUppass[posU]), prModel, w, mxTips);
            dfsRecalculateUppass(v2, &(prModel->parsVectUppass[posU]), prModel, w, mxTips);
        }
    }
}
/**
...
//            int posNz = width * states * Nz->number + w;
            std::size_t posNm = init(Nm->number);
            std::size_t posNs = init(Ns->number);
            std::size_t posNx = init(Nx->number);
            std::size_t posNz = init(Nz->number);
