...
struct NNIInfo {
  double lh_score[4]; // tree log-likelihood of zero-branch, current tree, NNI
                      // tree 1, NNI tree 2
  double br_len[4]; // length of current branch, optimized branch, NNI branch 1,
                    // NNI branch 2
  int nni_round;
...
struct Params {
  ...
  /**
          file containing site likelihood as input for 'guided bootstrap' (added
     by MA)
   */
  char *siteLL_file;

  /**
          alignment where the gappy patterns will be superimposed into the input
     alignment
   */
  char *gap_masked_aln;
  ...
  /** bootstrap specification of the form "l1:b1,l2:b2,...,lk:bk"
     ...
     error will occur. The default bootstrap_spec == NULL, a standard procedure
     is applied, i.e., randomly draw m sites.
  */
  char *bootstrap_spec;
  ...
  /**
          Percentage threshold to accept a branch of the approximate likelihood
     ratio test (aLRT) with SH-like interpretation. See Guindon et al. (2010)
     Syst. Biol. for details. Default: 90%.
   */
  int aLRT_threshold;
  ...
  */
  ...
  /**
              eco_detail_file - contains IDs of species present in the final set
     and/or species absent in the TREE or SPLIT system, but present in the food
     web
      */
  ...
  /*
              eco_weighted - indicates whether to treat the food web as weighted
     or not weighted
      */
  bool eco_weighted;
  ...
  /**
          TRUE to use ELW method of Strimmer & Rambaut for new bootstrap, FALSE
     otherwise
   */
  bool use_elw_method;
  ...
  /**
          TRUE to use the single ML tree per bootstrap, FALSE to include several
     sup-optima
   */
  ...
  int ratchet_percent; // 0-100; percent of informative sites to upweight during
                       // ratchet
  ...
  /*
   * Diep: to turn on the option for the 1st hill-climbing in ratchet iteration
   * by NNI
   */
  bool hclimb1_nni;

  /*
   * Diep: to turn off saveCurrentTree() during the 1st hill-climbing in ratchet
   * iteration
   */
  bool no_hclimb1_bb;
