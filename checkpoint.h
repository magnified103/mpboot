...
#define CKP_ARRAY_SAVE2(checkpoint, num, arr) \
    checkpoint->putArray(#arr, num, arr)
...
#define CKP_ARRAY_RESTORE2(checkpoint, num, arr) \
    checkpoint->getArray(#arr, num, arr)
...
public:
    explicit CkpStream(ios_base::openmode which = ios_base::in | ios_base::out)
        : stringstream(which)
    {
    }

    explicit CkpStream(const string& str,
...
public:
    ...
    void setIgnore(bool shouldIgnore)
    {
        ignore = shouldIgnore;
    }
    /**
    ...
    */
    ...
    /**
      ...
      @param compression true to compress checkpoint file, or false: no
     compression
    */
  ...
  */
    ...
    */
    ...
    /**
     * Save a map to the checkpoint
     * @param key key name
     * @param map map to save
     */
    template <typename U, typename V>
    void saveMap(const string& key, const unordered_map<U, V>& map);

    /**
     * Restore a map from the checkpoint
     * @param key key name
     * @param[out] map map to restore into
     */
    template <typename U, typename V>
    void restoreMap(const string& key, unordered_map<U, V>& map);
  ...
  */
   ...
   */
   ...
   @param overwrite true to overwrite value even if key exists
   */
    void transferSubCheckpoint(Checkpoint* source, string sub_key,
...
extern template void Checkpoint::saveMap<std::string, int>(const string&, const unordered_map<std::string, int>&);
extern template void Checkpoint::restoreMap<std::string, int>(const string&, unordered_map<std::string, int>&);
