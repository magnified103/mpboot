...
static inline void storePerSiteNodeScores2(partitionList *pr, int model,
    ...
    for (model = 0; model < pr->numberOfPartitions; model++) {
      ...
      case 2: {
        ...
        for (i = 0; i < width; i += INTS_PER_VECTOR) {
          ...
          if (perSiteScores) {
            storePerSiteNodeScores2(pr, model, v_N, i, pNumber, qNumber, rNumber);
          }
        }
      } break;
      case 4: {
...
template <class VectorClass, class Numeric, const size_t states,
          const bool BY_PATTERN, int perSiteScores>
parsimonyNumber evaluateSankoffParsimonyIterativeFastSIMD(pllInstance *tr,
  ...
  if (tr->ti[0] > 4)
    _newviewParsimonyIterativeFast<perSiteScores>(tr, pr);
...
//        return _evaluateSankoffParsimonyIterativeFast(tr, pr, perSiteScores);
    ...
    if (globalParam->sankoff_short_int) {
      ...
      case 4:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec16us, parsimonyNumberShort, 4, true, perSiteScores>(tr, pr);
      case 5:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec16us, parsimonyNumberShort, 5, true, perSiteScores>(tr, pr);
      case 20:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec16us, parsimonyNumberShort, 20, true, perSiteScores>(tr, pr);
      default:
        cerr << "Unsupported" << endl;
        exit(EXIT_FAILURE);
      }
      ...
      case 4:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec8ui, parsimonyNumber, 4, true, perSiteScores>(tr, pr);
      case 5:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec16us, parsimonyNumberShort, 5, true, perSiteScores>(tr, pr);
      case 20:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec8ui, parsimonyNumber, 20, true, perSiteScores>(tr, pr);
      default:
        cerr << "Unsupported" << endl;
        exit(EXIT_FAILURE);
      }
    ...
    if (globalParam->sankoff_short_int) {
      ...
      case 4:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec8us, parsimonyNumberShort, 4, true, perSiteScores>(tr, pr);
      case 20:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec8us, parsimonyNumberShort, 20, true, perSiteScores>(tr, pr);
      default:
        cerr << "Unsupported" << endl;
        exit(EXIT_FAILURE);
      }
      ...
      case 4:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec4ui, parsimonyNumber, 4, true, perSiteScores>(tr, pr);
      case 20:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec4ui, parsimonyNumber, 20, true, perSiteScores>(tr, pr);
      default:
        cerr << "Unsupported" << endl;
        exit(EXIT_FAILURE);
      }
...
void _newviewParsimonyIterativeFast(pllInstance *tr, partitionList *pr) {
  if (pllCostMatrix)
    return _newviewSankoffParsimonyIterativeFast<perSiteScores>(tr, pr);
  int model, *ti = tr->ti, count = ti[0], index;
  cout << "newhere\n";
  for (index = 4; index < count; index += 4) {
    unsigned int totalScore = 0;
...
unsigned int _evaluateParsimonyIterativeFast(pllInstance *tr, partitionList *pr) {
  ...
  if (pllCostMatrix)
    return _evaluateSankoffParsimonyIterativeFast<perSiteScores>(tr, pr);
...
unsigned int _evaluateParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  if (full) {
    if (p->number > tr->mxtips)
      computeTraversalInfoParsimony<perSiteScores>(p, ti, &counter, tr->mxtips, full);
    if (q->number > tr->mxtips)
      computeTraversalInfoParsimony<perSiteScores>(q, ti, &counter, tr->mxtips, full);
  } else {
...
template unsigned int _evaluateParsimony<0>(pllInstance *tr, partitionList *pr, nodeptr p,
                                pllBoolean full);
template unsigned int _evaluateParsimony<1>(pllInstance *tr, partitionList *pr, nodeptr p,
                                pllBoolean full);
...
void _newviewParsimony(pllInstance *tr, partitionList *pr, nodeptr p) {
  ...
  {
    int counter = 4;
    computeTraversalInfoParsimony<perSiteScores>(p, tr->ti, &counter, tr->mxtips, PLL_FALSE);
    tr->ti[0] = counter;
...
// template instantiation
template void _newviewParsimony<0>(pllInstance *tr, partitionList *pr, nodeptr p);
template void _newviewParsimony<1>(pllInstance *tr, partitionList *pr, nodeptr p);
...
static void testInsertParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  if (doIt) {
    ...
    insertParsimony<perSiteScores>(tr, pr, p, q);

    mp = _evaluateParsimony<perSiteScores>(tr, pr, p->next->next, PLL_FALSE);
...
static void addTraverseParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  if (doAll || (--mintrav <= 0))
    testInsertParsimony<perSiteScores>(tr, pr, p, q, saveBranches);

  if (((q->number > tr->mxtips)) && ((--maxtrav > 0) || doAll)) {
    addTraverseParsimony<perSiteScores>(tr, pr, p, q->next->back, mintrav, maxtrav, doAll,
                         saveBranches);
    addTraverseParsimony<perSiteScores>(tr, pr, p, q->next->next->back, mintrav, maxtrav,
                         doAll, saveBranches);
  }
}
...
static void addTraverseParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  if (found_better)
    return;
  if (doAll || (--mintrav <= 0))
    testInsertParsimony<perSiteScores>(tr, pr, p, q, saveBranches, found_better);

  if (((q->number > tr->mxtips)) && ((--maxtrav > 0) || doAll)) {
    addTraverseParsimony<perSiteScores>(tr, pr, p, q->next->back, mintrav, maxtrav, doAll,
                         saveBranches, found_better);
    addTraverseParsimony<perSiteScores>(tr, pr, p, q->next->next->back, mintrav, maxtrav,
                         doAll, saveBranches, found_better);
  }
}
...
static int rearrangeParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  unsigned int mp = _evaluateParsimony<perSiteScores>(
      tr, pr, p, PLL_FALSE); // Diep: This is VERY important to
  ...
  if ((p->number > tr->mxtips) && doP) {
    ...
    if ((p1->number > tr->mxtips) || (p2->number > tr->mxtips)) {
      ...
      if ((p1->number > tr->mxtips)) {
        addTraverseParsimony<perSiteScores>(tr, pr, p, p1->next->back, mintrav, maxtrav, doAll,
                             PLL_FALSE);
        addTraverseParsimony<perSiteScores>(tr, pr, p, p1->next->next->back, mintrav, maxtrav,
                             doAll, PLL_FALSE);
      }

      if ((p2->number > tr->mxtips)) {
        addTraverseParsimony<perSiteScores>(tr, pr, p, p2->next->back, mintrav, maxtrav, doAll,
                             PLL_FALSE);
        addTraverseParsimony<perSiteScores>(tr, pr, p, p2->next->next->back, mintrav, maxtrav,
                             doAll, PLL_FALSE);
      }
  ...
  if ((q->number > tr->mxtips) && (maxtrav > 0) && doQ) {
    ...
    if (((q1->number > tr->mxtips) &&
      ...
      if ((q1->number > tr->mxtips)) {
        addTraverseParsimony<perSiteScores>(tr, pr, q, q1->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE);
        addTraverseParsimony<perSiteScores>(tr, pr, q, q1->next->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE);
      }

      if ((q2->number > tr->mxtips)) {
        addTraverseParsimony<perSiteScores>(tr, pr, q, q2->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE);
        addTraverseParsimony<perSiteScores>(tr, pr, q, q2->next->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE);
      }
      ...
      _newviewParsimony<perSiteScores>(tr, pr, q);
    }
  }
...
static int rearrangeParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
                              ...
                              bool spr_better) {
  if (spr_better == false)
    return rearrangeParsimony<perSiteScores>(tr, pr, p, mintrav, maxtrav, doAll);
  nodeptr p1, p2, q, q1, q2;
  ...
  if ((p->number > tr->mxtips) && doP) {
    ...
    if ((p1->number > tr->mxtips) || (p2->number > tr->mxtips)) {
      ...
      if ((p1->number > tr->mxtips)) {
        addTraverseParsimony<perSiteScores>(tr, pr, p, p1->next->back, mintrav, maxtrav, doAll,
                             PLL_FALSE, found_better);
        if (found_better == false)
          addTraverseParsimony<perSiteScores>(tr, pr, p, p1->next->next->back, mintrav,
                               maxtrav, doAll, PLL_FALSE,
                               found_better);
      }

      if (((p2->number > tr->mxtips)) && (found_better == false)) {
        addTraverseParsimony<perSiteScores>(tr, pr, p, p2->next->back, mintrav, maxtrav, doAll,
                             PLL_FALSE, found_better);
        if (found_better == false)
          addTraverseParsimony<perSiteScores>(tr, pr, p, p2->next->next->back, mintrav,
                               maxtrav, doAll, PLL_FALSE,
                               found_better);
      }
  ...
  if ((q->number > tr->mxtips) && (maxtrav > 0) && doQ) {
    ...
    if (((q1->number > tr->mxtips) &&
      ...
      if ((q1->number > tr->mxtips)) {
        addTraverseParsimony<perSiteScores>(tr, pr, q, q1->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE, found_better);
        if (found_better == false)
          addTraverseParsimony<perSiteScores>(tr, pr, q, q1->next->next->back, mintrav2,
                               maxtrav, doAll, PLL_FALSE,
                               found_better);
      }

      if (((q2->number > tr->mxtips)) && found_better == false) {
        addTraverseParsimony<perSiteScores>(tr, pr, q, q2->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE, found_better);
        if (found_better == false)
          addTraverseParsimony<perSiteScores>(tr, pr, q, q2->next->next->back, mintrav2,
                               maxtrav, doAll, PLL_FALSE,
                               found_better);
      }
...
static void restoreTreeRearrangeParsimony(pllInstance *tr, partitionList *pr) {
  ...
  restoreTreeParsimony<perSiteScores>(tr, pr, tr->removeNode, tr->insertNode);
}
...
static void _pllMakeParsimonyTreeFast(pllInstance *tr, partitionList *pr,
  ...
  while (tr->ntips < tr->mxtips) {
...

    {
      ...
      computeTraversalInfoParsimony<0>(q, tr->ti, &counter, tr->mxtips, PLL_FALSE);
      tr->ti[0] = counter;
  ...
  do {
    ...
    //		for(j = 1; j <= tr->mxtips + tr->mxtips - 2; j++)
    {
      ...
      bestTreeScoreHits = 1;
      rearrangeParsimony<0>(tr, pr, tr->nodep[i], 1, sprDist, PLL_FALSE);
      ...
      if (((tr->bestParsimony < randomMP) ||
        ...
        restoreTreeRearrangeParsimony<0>(tr, pr);
        randomMP = tr->bestParsimony;
      }
    }
...
static void _pllMakeParsimonyTreeFast(pllInstance *tr, partitionList *pr,
                                      int sprDist, bool spr_better) {
  if (spr_better == false) {
    _pllMakeParsimonyTreeFast(tr, pr, sprDist);
    return;
  }
  nodeptr p, f;
  ...
  do {
    ...
    //		for(j = 1; j <= tr->mxtips + tr->mxtips - 2; j++)
    {
      ...
      tr->removeNode = tr->insertNode = NULL;
      bestTreeScoreHits = 1;
      rearrangeParsimony<0>(tr, pr, tr->nodep[i], 1, sprDist, PLL_FALSE, true);
      if (tr->bestParsimony == randomMP)
        bestIterationScoreHits++;
      if (tr->bestParsimony < randomMP)
        bestIterationScoreHits = 1;
...
void _pllComputeRandomizedStepwiseAdditionParsimonyTree(
  ...
  if (iqtree->params->spr_better == true) {
    _pllMakeParsimonyTreeFast(tr, partitions, sprDist, true);
  } else
...
int pllOptimizeSprParsimony(pllInstance *tr, partitionList *pr, int mintrav,
  ...
  if (perSiteScores) { // magnified: reduce branch misses
    tr->bestParsimony =
  ...
  */
  ...
  do {
    ...
    for (i = 2; i <= tr->mxtips + tr->mxtips - 2; i++) {
      ...
      if (perSiteScores) {
        rearrangeParsimony<true>(tr, pr, tr->nodep[i], mintrav, maxtrav, PLL_FALSE);
      } else {
        rearrangeParsimony<false>(tr, pr, tr->nodep[i], mintrav, maxtrav, PLL_FALSE);
      }
      if (tr->bestParsimony == randomMP)
        bestIterationScoreHits++;
      if (tr->bestParsimony < randomMP)
      ...
      if (((tr->bestParsimony < randomMP) ||
          ...
          tr->removeNode && tr->insertNode) {
        if (perSiteScores) {
          restoreTreeRearrangeParsimony<true>(tr, pr);
        } else {
          restoreTreeRearrangeParsimony<false>(tr, pr);
        }
        randomMP = tr->bestParsimony;
      }
    }
 ...
 * @param partition: the data partition :)
 * @param mintrav, maxtrav are PLL limitations for SPR radius
 * @param spr_better: take the first better or take the best
 * @return best parsimony score found
 */
int pllOptimizeSprParsimony(pllInstance *tr, partitionList *pr, int mintrav,
                            int maxtrav, IQTree *_iqtree, bool spr_better) {
  if (spr_better == false)
    return pllOptimizeSprParsimony(tr, pr, mintrav, maxtrav, _iqtree);
  int perSiteScores = globalParam->gbo_replicates > 0;
  ...
  */
  ...
  do {
    ...
    for (i = 1; i <= tr->mxtips + tr->mxtips - 2; i++) {
      ...
      if (perSiteScores) {
        rearrangeParsimony<true>(tr, pr, tr->nodep[i], mintrav, maxtrav, PLL_FALSE, true);
      } else {
        rearrangeParsimony<false>(tr, pr, tr->nodep[i], mintrav, maxtrav, PLL_FALSE, true);
      }
      if (tr->bestParsimony == randomMP)
        bestIterationScoreHits++;
      if (tr->bestParsimony < randomMP)
...
void computeUserTreeParsimomy(Params &params) {
  ...
  if (params.sankoff_cost_file) {
    ...
    unsigned int pll_score =
        _evaluateParsimony<false>(ptree->pllInst, ptree->pllPartitions,
                           ptree->pllInst->start, PLL_TRUE);
...
void testSPROnUserTree(Params &params) {
  ...
  rearrangeParsimony<0>(ptree->pllInst, ptree->pllPartitions,
                     ptree->pllInst->nodep[7], params.spr_mintrav,
                     params.spr_maxtrav, PLL_FALSE);
  if (ptree->pllInst->bestParsimony < curScore) {
    restoreTreeRearrangeParsimony<0>(ptree->pllInst, ptree->pllPartitions);
  }
