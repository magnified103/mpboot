...
//		return
//((cur_iteration+step_iteration-1)/step_iteration)*step_iteration; 	case
// SC_REAL_TIME:
...
bool StopRule::meetStopCondition(int cur_iteration, double cur_correlation) {
    ...
    case SC_BOOTSTRAP_CORRELATION:
        return ((cur_correlation >= min_correlation) &&
                (cur_iteration >
                 getLastImprovedIteration() + unsuccess_iteration)) ||
...
double StopRule::getRemainingTime(int cur_iteration, double cur_correlation) {
    ...
    case SC_WEIBULL:
        niterations =
            (predicted_iteration == 0) ? min_iteration : predicted_iteration;
        break;
    case SC_UNSUCCESS_ITERATION:
    ...
    case SC_ACO_UNSUCCESS_ITERATION:
        ...
        break;
    case SC_BOOTSTRAP_CORRELATION:
        niterations = ((cur_iteration + step_iteration - 1) / step_iteration) *
                      step_iteration;
...
// { 	this->min_correlation = min_correlation; 	this->step_iteration =
// step_iteration;
// }
//
    ...
    for (row_ = 0; row_ < nrow_; row_++)
        for (col_ = 0; col_ < ncol_; col_++) {
            ...
            for (int count_ = 0; count_ < mat1[0].size(); count_++) {
                ...
                //         std::cout << mat1[row_][count_] << " --> " <<
                //         mat2[count_][col_] << endl;
            }
        }
}
...
double StopRule::cmpMuy(int k) {
    ...
    for (int i = 0; i < k - 2; i++)
        sum_ += log((time_vec[0] - time_vec[k - 1]) /
                    (time_vec[0] - time_vec[i + 1]));
...
void StopRule::cmpLamdaMat(int k, DoubleMatrix &lamdaMat) {
    ...
    for (i = 0; i < k; i++)
        for (j = 0; j <= i; j++) {
            /*
            lamdaMat[i][j] = (cmpGamma (2*muy_ + i + 1) * cmpGamma (muy_ + j +
            1) ) / ( cmpGamma (muy_ + i + 1) * cmpGamma (j + 1) );*/

            // to fix divide by zero PROBLEM!
            lamdaMat[i][j] = cmpLnGamma(2 * muy_ + i + 1) +
                             cmpLnGamma(muy_ + j + 1) -
                             cmpLnGamma(muy_ + i + 1) - cmpLnGamma(j + 1);

            // if (i == 98 && j == 97)
            //      std::cout << i << "," << j << " -> " << lamdaMat[i][j] <<
            //      endl;
