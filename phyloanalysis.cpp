...
void reportReferences(Params &params, ofstream &out, string &original_model) {
    out << "To cite IQ-TREE please use:" << endl
        << endl
        << "Lam-Tung Nguyen, Heiko A. Schmidt, Arndt von Haeseler, and Bui "
           "Quang "
           "Minh (2014)"
        << endl
    ...
    if (params.gbo_replicates)
        out << "Since you also used ultrafast bootstrap (UFBoot) please cite: "
            << endl
            << endl
            << "Bui Quang Minh, Minh Anh Thi Nguyen, and Arndt von Haeseler "
               "(2013) "
               "Ultrafast"
            << endl
    ...
    /*		"*** If you use the parallel version, please cite: " << endl <<
     endl
     << "Bui Quang Minh, Le Sy Vinh, Arndt von Haeseler, and Heiko A. Schmidt
     (2005)" << endl << "pIQPNNI - parallel reconstruction of large maximum
     likelihood phylogenies." << endl << "Bioinformatics, 21:3794-3796." << endl
     << endl;*/

    // 	if (original_model == "TEST" || original_model == "TESTONLY")
    // 		out << "Since you used Modeltest please also cite Posada and
...
void pruneModelInfo(vector<ModelInfo> &model_info, PhyloSuperTree *tree) {
    ...
    for (vector<PartitionInfo>::iterator it = tree->part_info.begin();
         it != tree->part_info.end(); it++) {
        for (vector<ModelInfo>::iterator mit = model_info.begin();
             mit != model_info.end(); mit++)
...
void reportModelSelection(ofstream &out, Params &params,
                          vector<ModelInfo> &model_info, bool is_partitioned) {
    ...
    if (is_partitioned) {
        ...
        out << endl
            ...
            << ((params.model_test_criterion == MTC_BIC)
                    ? "BIC"
                    : ((params.model_test_criterion == MTC_AIC) ? "AIC"
                                                                : "AICc"))
            << " scores: " << endl
            << endl;
    }
    ...
    */
    ...
    for (it = model_info.begin(); it != model_info.end(); it++) {
        ...
        if (it != model_info.begin() && it->set_name != (it - 1)->set_name)
            setid++;
        if (is_partitioned && it != model_info.begin() &&
            it->set_name == (it - 1)->set_name)
            continue;
        if (is_partitioned) {
            out.width(4);
            out << right << setid << "  ";
    ...
    out << endl;
    out << "AIC, w-AIC   : Akaike information criterion scores and weights."
        << endl
...
void reportModel(ofstream &out, PhyloTree &tree) {
    ...
    if (tree.aln->num_states <= 4) {
        ...
        double *rate_mat =
            new double[tree.aln->num_states * tree.aln->num_states];
...
void reportRate(ofstream &out, PhyloTree &tree) {
    ...
    if (rate_model->getNDiscreteRate() > 1 || rate_model->getPInvar() > 0.0) {
        ...
        if (rate_model->getGammaShape() > 0 || rate_model->getPtnCat(0) < 0) {
            //			prop.resize(cats, (1.0 -
            // rate_model->getPInvar()) / rate_model->getNRate());
            prop.resize(cats);
            for (i = 0; i < cats; i++)
        ...
        if (rate_model->getGammaShape() > 0) {
            out << "Relative rates are computed as "
                << ((dynamic_cast<RateGamma *>(rate_model)->isCutMedian())
                        ? "MEDIAN"
                        : "MEAN")
                << " of the portion of the Gamma distribution falling in the "
                   "category."
                << endl;
        }
    }
    /*
     ...
     out << endl << "See file " << rate_file << " for site-specific rates and
     categories" << endl;*/
    out << endl;
}
...
void reportTree(ofstream &out, Params &params, PhyloTree &tree, double tree_lh,
    ...
    if (!params.maximum_parsimony) {
        out << "Total tree length (sum of branch lengths): " << totalLen
            << endl;
        ...
        int zero_internal_branches =
            tree.countZeroInternalBranches(NULL, NULL, epsilon);
        if (zero_internal_branches > 0) {
            // int zero_internal_branches = tree.countZeroInternalBranches(NULL,
            // NULL, epsilon);
            /*
        ...
        int long_branches =
            tree.countLongBranches(NULL, NULL, MAX_BRANCH_LEN - 0.2);
        if (long_branches > 0) {
            // stringstream sstr;
            out << "WARNING: " << long_branches << " too long branches (>"
                << MAX_BRANCH_LEN - 0.2 << ") should be treated with caution!"
                << endl;
            // out << sstr.str();
            // cout << sstr.str();
        }
    ...
    computeInformationScores(tree_lh, df, ssize, AIC_score, AICc_score,
                             BIC_score);
    ...
    if (params.maximum_parsimony) {
        ...
        out << "Log-likelihood of the tree: " << fixed << tree_lh << " (s.e. "
            ...
            << AICc_score << endl
            << "Bayesian information criterion (BIC) score: " << BIC_score
            << endl
...
void reportCredits(ofstream &out) {
    out << "CREDITS" << endl
        ...
        << endl
        << endl
        << "Schmidt HA, Strimmer K, Vingron M, and von Haeseler A (2002)"
        << endl
        ...
        //<< "The source code to construct the BIONJ tree were taken from BIONJ
        // software:"
        //<< endl << endl
        << "Gascuel O (1997) BIONJ: an improved version of the NJ algorithm"
        << endl
        << "based on a simple model of sequence data. Mol. Bio. Evol., "
           "14:685-695."
        << endl
        << endl
        ...
        //<< endl << endl
        << "Paul O. Lewis (2003) NCL: a C++ class library for interpreting "
           "data "
           "files in"
        << endl
        ...
        << "Mascagni M and Srinivasan A (2000) Algorithm 806: SPRNG: A "
           "Scalable "
           "Library"
        << endl
        << "for Pseudorandom Number Generation. ACM Transactions on "
           "Mathematical "
           "Software,"
        << endl
        << "26: 436-461." << endl
    ...
    << "David Posada and Keith A. Crandall (1998) MODELTEST: testing the model
    of"
...
void reportPhyloAnalysis(Params &params, string &original_model,
                         ...
                         vector<ModelInfo> &model_info, StrVector &removed_seqs,
                         StrVector &twin_seqs) {
    if (params.count_trees) {
        // addon: print #distinct trees
        cout << endl
             << "NOTE: " << pllTreeCounter.size()
        ...
        for (IntVector::iterator i2 = counts.begin(); i2 != counts.end();
             i2++) {
            if (*i2 != 0) {
                cout << "#Trees occuring " << (i2 - counts.begin())
                     << " times: " << *i2 << endl;
            }
        }
    }
    ...
    try {
        ...
        if (params.maximum_parsimony) {
            ...
            if (params.num_bootstrap_samples > 0) {
                out << " + ";
                out << "non-parametric bootstrap ("
                    << params.num_bootstrap_samples << " replicates)";
            }
            if (params.gbo_replicates > 0) {
                out << " + ";
                out << "MPBoot bootstrap approximation ("
        ...
        if (tree.isSuperTree()) {
            ...
            for (PhyloSuperTree::iterator it = stree->begin();
                ...
                // << endl
                // << endl; reportAlignment(out, *((*it)->aln));
                out.width(4);
                out << right << part + 1 << "  ";
        ...
        if (!params.maximum_parsimony) {
            ...
            if (tree.isSuperTree()) {
                if (params.partition_type)
                    out << "Proportional partition model with joint branch "
                           "lengths and "
                           "separate models between partitions"
                        << endl
                        << endl;
                else
                    out << "Full partition model with separate branch lengths "
                           "and models "
                           "between partitions"
                        << endl
                        << endl;
                ...
                // out << "-------------------------------------" << endl;
                for (it = stree->begin(), part = 0; it != stree->end();
                     it++, part++) {
                    out.width(4);
                    out << right << (part + 1) << "  ";
                    out.width(14);
                ...
                out << endl;
                /*
                for (it = stree->begin(), part = 0; it != stree->end(); it++,
                part++) { reportModel(out, *(*it)); reportRate(out, *(*it));
                }*/
            } else {
        ...
        */
        ...
        if (original_model == "WHTEST") {
            ...
            if (params.whtest_p_value < 0.05) {
                out << "RESULT: Homogeneity assumption is rejected (p-value "
                       "cutoff "
                       "0.05)"
                    << endl;
            } else {
                out << "RESULT: Homogeneity assumption is NOT rejected "
                       "(p-value cutoff "
                       "0.05)"
                    << endl;
            }
            out << endl << "*** For this result please cite:" << endl << endl;
            out << "G. Weiss and A. von Haeseler (2003) Testing substitution "
                   "models"
                << endl
        ...
        /*
                        out << "TREE SEARCH" << endl << "-----------" << endl <<
           endl
                                        << "Stopping rule: "
                                        << ((params.stop_condition ==
           SC_STOP_PREDICT) ? "Yes" : "No")
                                        << endl << "Number of iterations: "
                                        << tree.stop_rule.getNumIterations() <<
           endl
                                        << "Probability of deleting sequences: "
           << params.p_delete
                                        << endl << "Number of representative
           leaves:
           "
                                        << params.k_representative << endl
                                        << "NNI log-likelihood cutoff: " <<
           tree.getNNICutoff() << endl
                                        << endl;
        */
        if (params.compute_ml_tree) {
            out << (params.maximum_parsimony ? "MAXIMUM PARSIMONY TREE"
            ...
            if (params.partition_file)
                out << "NOTE: Branch lengths are weighted average over all "
                       "partitions"
                    << endl
                    << "      (weighted by the number of sites in the "
                       "partitions)"
                    << endl;
            ...
            if (tree.isSuperTree()) {
                ...
                if (empty_branches) {
                    stringstream ss;
                    ss << empty_branches
                       << " branches in the overall tree with no phylogenetic "
        ...
        /*
         ...
         out << endl << "CONSENSUS OF INTERMEDIATE TREES" << endl <<
         "-----------------------" << endl << endl
        ...
        if (params.consensus_type == CT_CONSENSUS_TREE) {
            ...
            out << "Consensus tree is constructed from "
                << (params.num_bootstrap_samples ? params.num_bootstrap_samples
            ...
            if (!params.maximum_parsimony)
                out << endl
                    << "Branch lengths are optimized by maximum likelihood on "
                       "original "
                       "alignment"
                    << endl;
            else
            ...
            //				tree.reinsertIdenticalSeqs(tree.aln,
            // removed_seqs, twin_seqs);
            //			}
            //			tree.setAlignment(tree.aln);

            // bug fix
            //			if ((tree.sse == LK_EIGEN || tree.sse ==
            // LK_EIGEN_SSE) && !tree.isBifurcating()) {
            // cout << "NOTE: Changing to old kernel as consensus tree is
            // multifurcating" << endl;
            ...
            //			tree.printTree(con_file.c_str(), WT_BR_LEN |
            // WT_BR_LEN_FIXED_WIDTH | WT_SORT_TAXA);
            // tree.sortTaxa(); 			tree.drawTree(out,
            // WT_BR_SCALE); out << endl <<
            //"Consensus tree in newick format: " << endl << endl;
            // tree.printResultTree(out); 			out << endl <<
            // endl;
        }
        ...
        if (params.treeset_file) {
            ...
            out << "See " << params.treeset_file
                << ".trees for trees with branch lengths." << endl
                << endl;
            ...
            for (tid = 0; tid < info.size(); tid++)
                if (info[tid].logl > maxL)
                    maxL = info[tid].logl;
            for (orig_id = 0, tid = 0; orig_id < distinct_trees.size();
                 orig_id++) {
                out.width(3);
                out << right << orig_id + 1 << " ";
            ...
            if (params.topotest_replicates) {
                out << "deltaL  : logL difference from the maximal logl in the "
                       "set."
                    << endl
                ...
                out << "c-ELW   : Expected Likelihood Weight (Strimmer & "
                       "Rambaut 2002)."
                    << endl
                    << endl
                    << "Plus signs denote the 95% confidence sets." << endl
                    << "Minus signs denote significant exclusion." << endl
                    << "All tests performed " << params.topotest_replicates
                    << " resamplings using the RELL method." << endl;
            }
            out << endl;
        }
        ...
        out << "TIME STAMP" << endl
            ...
            << "Total CPU time used: " << (double)params.run_time
            << " seconds (" << convert_time(params.run_time) << ")" << endl
            << "Total wall-clock time used: "
            << getRealTime() - params.start_real_time << " seconds ("
    ...
    cout << endl
         ...
         << "  MPBoot report:                 " << params.out_prefix
         << ".mpboot" << endl;
    if (params.compute_ml_tree) {
        cout << "  Maximum-"
             << (params.maximum_parsimony ? "parsimony " : "likelihood")
             << " tree:       " << params.out_prefix << ".treefile" << endl;
        if (params.write_local_optimal_trees)
            cout << "  Candidate trees (" << tree.candidateTrees.size()
                 << "):      " << params.out_prefix << ".localtrees" << endl;
    }
    if (!params.user_file && params.start_tree == STT_BIONJ) {
        cout << "  BIONJ tree:                    " << params.out_prefix
             << ".bionj" << endl;
    }
    if (!params.dist_file) {
        // cout << "  Juke-Cantor distances:    " << params.out_prefix <<
        // ".jcdist"
        // << endl;
    ...
    if (tree.getRate()->getGammaShape() > 0 && params.print_site_rate)
        cout << "  Gamma-distributed rates:       " << params.out_prefix
             << ".rate" << endl;

    if ((tree.getRate()->isSiteSpecificRate() ||
         tree.getRate()->getPtnCat(0) >= 0) &&
        params.print_site_rate)
        cout << "  Site-rates by MH model:        " << params.out_prefix
             << ".rate" << endl;

    if (params.print_site_lh)
        cout << "  Site log-likelihoods:          " << params.out_prefix
             << ".sitelh" << endl;

    if (params.print_site_pars)
        cout << "  Site parsimony scores:         " << params.out_prefix
             << ".sitepars" << endl;

    if (params.write_intermediate_trees)
        cout << "  All intermediate trees:        " << params.out_prefix
             << ".treels" << endl;

    if (params.gbo_replicates) {
        if (params.maximum_parsimony)
            cout << endl
                 << "Maximum parsimony bootstrap approximation results written "
                    "to:"
                 << endl
                 << "  Split support values:          " << params.out_prefix
                 << ".splits.nex" << endl
                 << "  Consensus tree:                " << params.out_prefix
                 << ".contree" << endl;
        else
            cout << endl
        ...
        if (params.print_ufboot_trees)
            cout << "  UFBoot trees:                  " << params.out_prefix
                 << ".ufboot" << endl;
    }

    if (params.treeset_file) {
        cout << "  Evaluated user trees:          " << params.out_prefix
             << ".trees" << endl;

        if (params.print_tree_lh) {
            cout << "  Tree log-likelihoods:          " << params.out_prefix
                 << ".treelh" << endl;
        }
        if (params.print_site_lh) {
            cout << "  Site log-likelihoods:          " << params.out_prefix
    ...
    cout << "  Screen log file:               " << params.out_prefix << ".log"
         << endl;
    /*	if (original_model == "WHTEST")
     cout <<"  WH-TEST report:           " << params.out_prefix << ".whtest" <<
     endl;*/
    cout << endl;
}
    ...
    for (i = 0; i < ntaxa - 1; i++) {
        ...
        for (j = i + 1; j < ntaxa; j++)
            if (dist[i * ntaxa + j] <= 1e-6) {
                if (first)
                    str =
                        "ZERO distance between sequences " + aln->getSeqName(i);
...
void printAnalysisInfo(int model_df, IQTree &iqtree, Params &params) {
    ...
    if (iqtree.isSuperTree()) {
        cout << iqtree.getModelName() << " (" << model_df << " free parameters)"
             << endl;
    } else {
    ...
    if (params.min_iterations > 0) {
        cout << "Tree search algorithm: "
             << (params.snni ? "Stochastic nearest neighbor interchange"
                             : "IQPNNI")
             << endl;
        ...
        if (!params.snni) {
            ...
            cout << "Number of leaves to be deleted   : " << iqtree.getDelete()
                 << endl;
    ...
    cout << "Phylogenetic likelihood library: " << (params.pll ? "Yes" : "No")
         << endl;
    cout << "Branch length optimization method: "
         << ((iqtree.optimize_by_newton) ? "Newton" : "Brent") << endl;
    cout
        << "Number of Newton-Raphson steps in NNI evaluation and branch length "
           "optimization: "
        << NNI_MAX_NR_STEP << " / " << PLL_NEWZPERCYCLE << endl;
    cout << "SSE instructions: " << ((iqtree.sse) ? "Yes" : "No") << endl;
    if (params.maximum_parsimony && params.spr_parsimony) {
        cout << "Hill-climbing using SPR radius: " << params.spr_maxtrav
             << endl;
    }
    cout << endl;
}

void computeMLDist(Params &params, IQTree &iqtree, string &dist_file,
                   double begin_time, double &bestTreeScore) {
    double longest_dist;
    ...
    double *ml_var = NULL;
    longest_dist =
        iqtree.computeDist(params, iqtree.aln, ml_dist, ml_var, dist_file);
    ...
    } // else
    {
        if (!iqtree.dist_matrix) {
            iqtree.dist_matrix =
                new double[iqtree.aln->getNSeq() * iqtree.aln->getNSeq()];
        }
...
void computeInitialDist(Params &params, IQTree &iqtree, string &dist_file) {
    ...
    if (params.compute_jc_dist || params.compute_obs_dist ||
        ...
        longest_dist =
            iqtree.computeDist(params, iqtree.aln, iqtree.dist_matrix,
...
void computeInitialTree(Params &params, IQTree &iqtree, string &dist_file,
                        int &numInitTrees, string &initTree) {
    ...
    if (params.user_file) {
        ...
        iqtree.clearAllPartialLH(); // 2020-08-17: Diep added to fix bug while
                                    // compute score of user tree
        ...
        if ((params.SSE == LK_EIGEN || params.SSE == LK_EIGEN_SSE) &&
            ...
            cout << "NOTE: Changing to old kernel as input tree is "
                    "multifurcating"
                 << endl;
            params.SSE = LK_SSE;
        }
        ...
        case STT_PARSIMONY:
            ...
            cout << endl;
            cout << "Creating initial parsimony tree by random order stepwise "
                    "addition..."
                 << endl;
            ...
            if (numInitTrees > params.min_iterations &&
                params.stop_condition == SC_FIXED_ITERATION)
                numInitTrees = params.min_iterations;
            break;
        case STT_PLL_PARSIMONY:
            cout << endl;
            cout << "Create initial parsimony tree by phylogenetic likelihood "
                    "library (PLL)... ";
            ...
            if (params.maximum_parsimony) {
                _pllComputeRandomizedStepwiseAdditionParsimonyTree(
                    iqtree.pllInst, iqtree.pllPartitions, params.sprDist,
                    &iqtree);
            } else {
    ...
    if (fixed_number) {
        cout << "WARNING: " << fixed_number
             << " undefined/negative branch lengths are initialized with "
                "parsimony"
             << endl;
    }
    if (params.root) {
...
void initializeParams(Params &params, IQTree &iqtree,
    ...
    if (params.model_name.substr(0, 4) == "TEST") {
        ...
        cout << "NOTE: MODEL SELECTION REQUIRES AT LEAST "
             << ((double)mem_size * sizeof(double) / 1024.0) / 1024
             << " MB MEMORY!" << endl;
        if (mem_size >= getMemorySize()) {
            outError("Memory required exceeds your computer RAM size!");
        }
        params.model_name = testModel(params, &iqtree, model_info);
        cout << "CPU time for model selection: "
             << getCPUTime() - params.startCPUTime << " seconds." << endl;
    ...
    if (params.model_name == "WHTEST") {
        if (iqtree.aln->seq_type != SEQ_DNA)
            outError("Weiss & von Haeseler test of model homogeneity only "
                     "works for DNA");
...
int initCandidateTreeSet(Params &params, IQTree &iqtree, int numInitTrees) {
    ...
    int numDupPars = 0;
    //    if(params.maximum_parsimony) iqtree.candidateTrees.clear(); // Diep:
    //    added this to fix the bug of sorted aln <> orig aln
    for (int treeNr = 1; treeNr < numInitTrees; treeNr++) {
        ...
        if (params.start_tree == STT_PLL_PARSIMONY) {
            ...
            if (params.maximum_parsimony) {
                ...
                if (params.tbr_init) {
                    pllComputeRandomizedStepwiseAdditionParsimonyTreeTBR(
                        iqtree.pllInst, iqtree.pllPartitions, 1, 4, &iqtree);
                } else
                    _pllComputeRandomizedStepwiseAdditionParsimonyTree(
                        iqtree.pllInst, iqtree.pllPartitions, params.sprDist,
                        &iqtree);
    ...
    // do not do anything for parsimony because tree was already optimized by
    // SPR
    if (params.maximum_parsimony) {
        return -1;
    }
    ...
    vector<string> unOptParTrees =
        iqtree.candidateTrees.getHighestScoringTrees(numInitTrees);
    ...
    CandidateSet initParsimonyTrees =
        iqtree.candidateTrees.getBestCandidateTrees(params.numNNITrees);
    iqtree.candidateTrees.clear();
    if (verbose_mode >= VB_MED) {
        for (multimap<double, CandidateTree>::iterator it =
                 iqtree.candidateTrees.begin();
             it != iqtree.candidateTrees.end(); it++) {
    ...
    /************ END: Create a set of up to (numInitTrees - 1) unique parsimony
     * trees **********************/

    cout << endl;
    cout << "Optimizing top " << initParsimonyTrees.size()
         << " parsimony trees with NNI..." << endl;
    startTime = getCPUTime();
    ...
    for (rit = initParsimonyTrees.rbegin(); rit != initParsimonyTrees.rend();
         ++rit, iqtree.setCurIt(iqtree.getCurIt() + 1)) {
    ...
    cout << "Average time for 1 NNI search: "
...
void pruneTaxa(Params &params, IQTree &iqtree, double *pattern_lh,
    ...
    if (params.aLRT_threshold <= 100 &&
        ...
        num_low_support = iqtree.testAllBranches(
            params.aLRT_threshold, iqtree.curScore, pattern_lh,
...
void restoreTaxa(IQTree &iqtree, double *saved_dist_mat,
    ...
    if (!pruned_taxa.empty()) {
        ...
        iqtree.curScore = iqtree.optimizeNNI(nni_count, nni_steps);
        cout
            << "Log-likelihood	after reoptimizing full tree: "
            << iqtree.curScore
            << endl; // iqtree.setBestScore(iqtree.getModelFactory()->optimizeParameters(params.fixed_branch_length,
...
void runApproximateBranchLengths(Params &params, IQTree &iqtree) {
    if (!params.fixed_branch_length && params.leastSquareBranch) {
        ...
        cout << "Logl of tree with LS branch lengths: " << iqtree.curScore
             << endl;
    ...
    if (params.pars_branch_length) {
        ...
        iqtree.printTree(filename.c_str(), WT_BR_LEN | WT_BR_LEN_FIXED_WIDTH |
                                               WT_SORT_TAXA | WT_NEWLINE);
        cout << "Logl of tree with MP branch lengths: " << iqtree.curScore
             << endl;
    ...
    if (params.bayes_branch_length) {
        ...
        iqtree.printTree(filename.c_str(), WT_BR_LEN | WT_BR_LEN_FIXED_WIDTH |
                                               WT_SORT_TAXA | WT_NEWLINE);
        cout << "Logl of tree with Bayesian branch lengths: " << iqtree.curScore
             << endl;
        cout << "Tree with Bayesian branch lengths written to " << filename
             << endl;
        if (params.print_branch_lengths) {
            ofstream out;
            filename = params.out_prefix;
...
void printMiscInfo(Params &params, IQTree &iqtree, double *pattern_lh) {
    ...
    if (params.mvh_site_rate) {
        RateMeyerHaeseler *rate_mvh = new RateMeyerHaeseler(
            params.rate_file, &iqtree, params.rate_mh_type);
        cout << endl
             << "Computing site-specific rates by " << rate_mvh->full_name
             << "..." << endl;
        rate_mvh->runIterativeProc(params, iqtree);
        cout << endl
             << "BEST SCORE FOUND : "
             << (params.maximum_parsimony ? -iqtree.getBestScore()
                                          : iqtree.getBestScore())
             << endl;
    ...
    if (params.print_site_rate) {
        ...
        if (iqtree.isSuperTree()) {
            ...
            try {
                ...
                for (PhyloSuperTree::iterator it = stree->begin();
                     it != stree->end(); it++, part++) {
                    out << "SITE RATES FOR PARTITION "
                        << stree->part_info[part].name << ":" << endl;
...
void printFinalSearchInfo(Params &params, IQTree &iqtree,
    ...
    if (iqtree.isSuperTree()) {
        PhyloSuperTree *stree = (PhyloSuperTree *)&iqtree;
        cout << stree->evalNNIs << " NNIs evaluated from " << stree->totalNNIs
             << " all possible NNIs ( "
             << (int)(((stree->evalNNIs + 1.0) / (stree->totalNNIs + 1.0)) *
                      100.0)
             << " %)" << endl;
        cout << "Details for subtrees:" << endl;
        for (int part = 0; part < stree->size(); part++) {
            cout << part + 1 << ". " << stree->part_info[part].name << ": "
                 << stree->part_info[part].evalNNIs << " ( "
                 << (int)(((stree->part_info[part].evalNNIs + 1.0) /
    ...
    cout.precision(3);
    cout << endl;
    cout << "CPU time used for tree search: " << search_cpu_time << " sec ("
         << convert_time(search_cpu_time) << ")" << endl;
...
void runTreeReconstruction(Params &params, string &original_model,
    ...
    // Make sure that no partial likelihood of IQ-TREE is initialized when PLL
    // is used to save memory
    if (params.pll) {
        iqtree.deleteAllPartialLh();
    }
    // Temporary fix since PLL only supports DNA/Protein: switch to IQ-TREE
    // parsimony kernel
    if (params.start_tree == STT_PLL_PARSIMONY) {
        if (iqtree.isSuperTree()) {
            PhyloSuperTree *stree = (PhyloSuperTree *)&iqtree;
...
#else
        cout << "NOTE: " << ((double)mem_size * sizeof(double) / 1000.0) / 1000
             << " MB RAM is required!" << endl;
#endif
        if (mem_size >= getMemorySize()) {
            outError("Memory required exceeds your computer RAM size!");
    ...
    // Optimize model parameters and branch lengths using ML for the initial
    // tree
    if (!params.maximum_parsimony)
    ...
    cout << "Current best tree score: "
         << (params.maximum_parsimony ? -iqtree.bestScore : iqtree.bestScore)
         << endl
         << endl;
    ...
    if (params.min_iterations > 0) {
        ...
        if (!params.user_file && (params.start_tree == STT_PARSIMONY ||
            ...
            cout << "Number of distinct locally optimal trees: "
                 << iqtree.candidateTrees.size() << endl;
        } else {
            int nni_count = 0;
            int nni_steps = 0;
            ...
            // Because the following doNNISearch does not have ratchet alignment
            // setup yet, we have to turn-off ratchet
            int saved_ratchet_iter = params.ratchet_iter;
            params.ratchet_iter = -1;
            string tree = iqtree.doNNISearch(nni_count, nni_steps);
            params.ratchet_iter =
                saved_ratchet_iter; // Now, reset ratchet
                                    //            if (params.pll) {
            //                iqtree.curScore = iqtree.pllOptimizeNNI(nni_count,
            //                nni_steps, iqtree.searchinfo);
            //                pllTreeToNewick(iqtree.pllInst->tree_string,
            //                iqtree.pllInst, iqtree.pllPartitions,
            //                        iqtree.pllInst->start->back, PLL_TRUE,
            //                        PLL_TRUE, PLL_FALSE, PLL_FALSE, PLL_FALSE,
            //                        PLL_SUMMARIZE_LH, PLL_FALSE, PLL_FALSE);
            //                iqtree.setBestTree(string(iqtree.pllInst->tree_string),
            //                iqtree.curScore);
            //            } else {
            //                iqtree.curScore = iqtree.optimizeNNI(nni_count,
            //                nni_steps);
            //                iqtree.setBestTree(iqtree.getTreeString(),
            //                iqtree.curScore);
            //            }
            //
            //        	if (iqtree.isSuperTree())
            //        		((PhyloSuperTree*)
            //        &iqtree)->computeBranchLengths();
            iqtree.setBestTree(tree, iqtree.curScore);
            iqtree.candidateTrees.update(tree, iqtree.curScore);
        }

        cout << "Current best score: "
             << (params.maximum_parsimony ? -iqtree.bestScore
                                          : iqtree.bestScore)
             << " / CPU time: " << getCPUTime() - initTime << endl
             << endl;
    }
    ...
    if (params.min_iterations > 1) {
        ...
        if (params.tree_spr) {
            ...
            // double spr_score = tree.optimizeSPR(tree.curScore, (PhyloNode*)
            // tree.root->neighbors[0]->node);
    ...
    if (params.snni && params.min_iterations) {
        cout << (params.maximum_parsimony ? "Scores" : "Log-likelihoods")
             << " of best " << params.popSize << " trees: " << endl;
    ...
    if (params.write_local_optimal_trees) {
        ...
        ofstream treesOut((string(params.out_prefix) + ".localtrees").c_str(),
                          ofstream::out);
        for (vector<string>::iterator it = trees.begin(); it != trees.end();
             it++)
            treesOut << (*it) << endl;
    }
    ...
    if ((params.aLRT_replicates > 0 || params.localbp_replicates > 0) &&
        ...
        cout << endl
             ...
             << params.aLRT_replicates << " replicates..." << endl;
        iqtree.setRootNode(params.root);
        iqtree.testAllBranches(params.aLRT_threshold, myscore, pattern_lh,
                               params.aLRT_replicates,
    ...
    // To free data structures of PLL parsimony at the very end if NOT running
    // ratchet or NOT -opt_btree
    if (params.maximum_parsimony && params.spr_parsimony &&
        (params.ratchet_iter < 0) && (!params.optimize_boot_trees) &&
        (params.snni || params.pll)) {
 ...
 ***********************************************************/
void runStandardBootstrap(Params &params, string &original_model,
                          Alignment *alignment, IQTree *tree) {
    vector<ModelInfo> model_info;
    ...
    for (int sample = 0; sample < params.num_bootstrap_samples; sample++) {
        cout << endl
             << "===> START BOOTSTRAP REPLICATE NUMBER " << sample + 1 << endl
             << endl;
        ...
        if (alignment->isSuperAlignment()) {
            if (params.partition_type) {
                ...
                boot_tree =
                    new PhyloSuperTree((SuperAlignment *)bootstrap_alignment,
                                       (PhyloSuperTree *)tree);
            }
        } else
        ...
        if (params.num_bootstrap_samples == 1)
            reportPhyloAnalysis(params, original_model, *bootstrap_alignment,
                                *boot_tree, model_info, removed_seqs,
                                twin_seqs);
    ...
    if (params.compute_ml_tree) {
        ...
        cout << endl
             << "===> ASSIGN BOOTSTRAP SUPPORTS TO THE TREE FROM ORIGINAL "
                "ALIGNMENT"
             << endl
             << endl;
        MExtTree ext_tree;
        assignBootstrapSupport(
            boottrees_name.c_str(), 0, 1e6, treefile_name.c_str(), false,
            treefile_name.c_str(), params.out_prefix, ext_tree, NULL, &params);
    ...
    if (params.print_bootaln)
        cout << "  Bootstrap alignments:     " << params.out_prefix
             << ".bootaln" << endl;
    cout << "  Bootstrap trees:          " << params.out_prefix << ".boottrees"
         << endl;
    if (params.consensus_type == CT_CONSENSUS_TREE)
...
void convertAlignment(Params &params, IQTree *iqtree) {
    ...
    if (alignment->isSuperAlignment()) {
        ((SuperAlignment *)alignment)
            ->printCombinedAlignment(params.aln_output);
        if (params.print_subaln)
            ((SuperAlignment *)alignment)
                ->printSubAlignments(params,
                                     ((PhyloSuperTree *)iqtree)->part_info);
        ...
        out_aln.printPhylip(params.aln_output, false, params.aln_site_list,
                            params.aln_nogaps, params.aln_no_const_sites,
                            params.ref_seq_name);
...
void runPhyloAnalysis(Params &params) {
    ...
    if (params.partition_file) {
        ...
        if (params.partition_type) {
            ...
            if (params.nni5)
                outError("-nni5 option is unsupported yet for proportitional "
                         "partition "
                         "model. please use -nni1 option");
            if (params.aLRT_replicates)
                outError("-alrt option is unsupported yet for proportitional "
                         "partition "
                         "model");
            // initialize supertree - Proportional Edges case, "-spt p" option
            tree = new PhyloSuperTreePlen(params);
        } else {
            // initialize supertree stuff if user specifies partition file with
            // -sp option
        ...
        if (params.maximum_parsimony && !params.sankoff_cost_file &&
            params.condense_parsimony_equiv_sites) {
    ...
    if (params.maximum_parsimony &&
        ...
        optimizeAlignment(tree,
                          params); // Diep: this is to rearrange columns for
                                   // better speed in REPS
    }
    ...
    if (params.aln_output) {
        ...
        if (params.gbo_replicates && params.online_bootstrap) {
            ...
            computeConsensusTree(splitsfile.c_str(), 0, 1e6,
                                 params.split_threshold,
                                 params.split_weight_threshold, NULL,
                                 params.out_prefix, NULL, &params);
            ...
            // bug fix
            if ((tree->sse == LK_EIGEN || tree->sse == LK_EIGEN_SSE) &&
                !tree->isBifurcating()) {
                cout << "NOTE: Changing to old kernel as consensus tree is "
                        "multifurcating"
                     << endl;
            ...
            //	        		PhyloSuperTree *stree =
            //(PhyloSuperTree*) tree; tree->clearAllPartialLH();
            ...
            if (params.maximum_parsimony)
                ...
                cout << "Log-likelihood of consensus tree: " << conScore
                     << endl;
            tree->setRootNode(params.root);
            tree->insertTaxa(removed_seqs, twin_seqs);
            //			tree->printTree(splitsfile.c_str(), WT_BR_LEN |
            // WT_BR_LEN_FIXED_WIDTH | WT_SORT_TAXA | WT_NEWLINE); // for ML
            tree->printTree(splitsfile.c_str(),
...
void printSiteParsimonyUserTree(Params &params) {
    ...
    ptree->params = &params; // Diep: 2020-08-17, there are two variables with
...
void assignBranchSupportNew(Params &params) {
    ...
    cout << tree.leafNum << " taxa and " << tree.branchNum << " branches"
         << endl;
...
/**
 * assign split occurence frequencies from a set of input trees onto a target
 * tree NOTE: input trees must have the same taxon set
 ...
 * @param output_file file name to write output tree with assigned support
 * values
 * @param out_prefix prefix of output file
 * @param mytree (OUT) resulting tree with support values assigned from
 * target_tree
...
void assignBootstrapSupport(const char *input_trees, int burnin, int max_count,
                            const char *target_tree, bool rooted,
                            const char *output_tree, const char *out_prefix,
                            MExtTree &mytree, const char *tree_weight_file,
                            Params *params) {
    // bool rooted = false;
    //  read the tree file
...
void computeConsensusTree(const char *input_trees, int burnin, int max_count,
                          double cutoff, double weight_threshold,
                          const char *output_tree, const char *out_prefix,
    ...
    if (params && detectInputFile((char *)input_trees) == IN_NEXUS) {
        ...
        /*		StrVector sgtaxname;
         sg.getTaxaName(sgtaxname);
         i = 0;
         for (StrVector::iterator sit = sgtaxname.begin(); sit !=
         sgtaxname.end(); sit++, i++) { Node *leaf = mytree.findLeafName(*sit);
         if (!leaf) outError("Tree does not contain taxon ", *sit); leaf->id =
         i;
         }*/
        scale /= sg.maxWeight();
    } else {
    ...
    if (params->print_splits_file) {
        ...
        cout << "Non-trivial split supports printed to star-dot file "
             << out_file << endl;
    }
}

void testCompConsensus(const char *infile, const char *outfile,
                       Params *params) {
    StringIntMap treels;
    int index = 0;
    ifstream fin(infile);
    ...
    string con_str = computeConsensusTreeNoFileIO(
        treels, weight, params->tree_max_count, params->split_threshold,
        params->split_weight_threshold, params);
...
string computeConsensusTreeNoFileIO(StringIntMap &input_trees,
                                    IntVector &weight, int max_count,
                                    double cutoff, double weight_threshold,
                                    Params *params) {
    bool rooted = false;
    ...
    ostringstream ostr;
    string tree_str;
    //    mytree.printTree(ostr, WT_BR_CLADE); // for testing by
    //    testCompConsensus
...
void computeConsensusNetwork(const char *input_trees, int burnin, int max_count,
                             double cutoff, int weight_summary,
...
void optimizeAlignment(IQTree *&tree, Params &params) {
    ...
    if (params.user_file) {
        ...
        cout << "\nComputing random stepwise addition parsimony tree for "
                "sorting "
                "patterns..."
             << endl;
        // tree->initTopologyByPLLRandomAdition(params); // this pll version
        // needs further sync to work with the rest
        tree->computeParsimonyTree(
            params.out_prefix,
            tree->aln); // this iqtree version plays nicely with the rest
        cout
    ...
    //	if(checkDuplicatePattern(tree))
    //		cout << "SECOND CHECK: Sorted alignment patterns are duplicate!"
    //<< endl; 	exit(0);

    //	for(int i = 0; i < tree->getAlnNPattern(); i++){
    //		if(i == 253 || i == 254)
    //			cout << "i = " << i << ", score = " <<
    //(tree->aln)->at(i).ras_pars_score << endl;
    //	}
    //	exit(0);
    ...
    //	Alignment * alignment = new Alignment("temp.phy", params.sequence_type,
    // params.intype);
