...
void reportModelSelection(ofstream &out, Params &params,
  ...
  if (is_partitioned)
    out << "  ID  ";
  out << "Model             LogL          AIC      w-AIC      AICc     w-AICc  "
         "     BIC      w-BIC"
      << endl;
  /*
  if (is_partitioned)
  ...
  if (rate_model->getNDiscreteRate() > 1 || rate_model->getPInvar() > 0.0) {
    ...
    if (rate_model->getGammaShape() > 0) {
      out << "Relative rates are computed as "
          << ((dynamic_cast<RateGamma *>(rate_model)->isCutMedian()) ? "MEDIAN"
                                                                     : "MEAN")
...
void reportTree(ofstream &out, Params &params, PhyloTree &tree, double tree_lh,
    ...
    if (zero_internal_branches > 0) {
      ...
      */
      ...
      and should be treated with caution!"
                      << endl;
      */
      out << "         Such branches are denoted by '**' in the figure" << endl
          << endl;
    }
    int long_branches =
  ...
  if (params.maximum_parsimony) {
    ...
    out << "Log-likelihood of the tree: " << fixed << tree_lh << " (s.e. "
        ...
        << "Corrected Akaike information criterion (AICc) score: " << AICc_score
        << endl
...
void reportCredits(ofstream &out) {
  out << "CREDITS" << endl
      ...
      //<< endl << endl
      << "Paul O. Lewis (2003) NCL: a C++ class library for interpreting data "
         "files in"
      << endl
      ...
      << "Mascagni M and Srinivasan A (2000) Algorithm 806: SPRNG: A Scalable "
         "Library"
      << endl
      << "for Pseudorandom Number Generation. ACM Transactions on Mathematical "
         "Software,"
      << endl
      << "26: 436-461." << endl
      << endl
...
void reportPhyloAnalysis(Params &params, string &original_model,
  ...
  if (params.count_trees) {
    ...
    for (IntVector::iterator i2 = counts.begin(); i2 != counts.end(); i2++) {
      if (*i2 != 0) {
        cout << "#Trees occuring " << (i2 - counts.begin()) << " times: " << *i2
             << endl;
      }
    }
  }
  ...
  try {
    ...
    if (tree.isSuperTree()) {
      out << "Input data: " << alignment.getNSeq() << " taxa with "
          << alignment.getNSite() << " partitions and " << tree.getAlnNSite()
          << " total sites ("
      ...
      for (PhyloSuperTree::iterator it = stree->begin(); it != stree->end();
           it++, part++) {
        // out << "FOR PARTITION " << stree->part_info[part].name << ":" << endl
    ...
    if (!params.maximum_parsimony) {
      ...
      if (tree.isSuperTree()) {
        if (params.partition_type)
          out << "Proportional partition model with joint branch lengths and "
                 "separate models between partitions"
              << endl
              << endl;
        else
          out << "Full partition model with separate branch lengths and models "
                 "between partitions"
              << endl
              << endl;
        PhyloSuperTree *stree = (PhyloSuperTree *)&tree;
    ...
    */
    ...
    if (original_model == "WHTEST") {
      out << "TEST OF MODEL HOMOGENEITY" << endl
          << "-------------------------" << endl
          << endl;
      out << "Delta of input data:                 " << params.whtest_delta
          << endl;
      ...
      out << "P-value:                             " << params.whtest_p_value
          << endl;
      if (params.whtest_p_value < 0.05) {
        out << "RESULT: Homogeneity assumption is rejected (p-value cutoff "
               "0.05)"
            << endl;
      } else {
        out << "RESULT: Homogeneity assumption is NOT rejected (p-value cutoff "
               "0.05)"
            << endl;
      }
      out << endl << "*** For this result please cite:" << endl << endl;
      out << "G. Weiss and A. von Haeseler (2003) Testing substitution models"
          << endl
    ...
    */
    ...
    /*
     ...
     "-----------------------" << endl << endl
     << "Number of intermediate trees: " << tree.stop_rule.getNumIterations() <<
     endl
    ...
    if (params.consensus_type == CT_CONSENSUS_TREE) {
      ...
      if (!params.maximum_parsimony)
        out << endl
            << "Branch lengths are optimized by maximum likelihood on original "
               "alignment"
            << endl;
      else
        out << endl;
      ...
      // tree.sortTaxa(); 			tree.drawTree(out, WT_BR_SCALE);
      // out << endl <<
      //"Consensus tree in newick format: " << endl << endl;
      // tree.printResultTree(out); 			out << endl << endl;
    }
    ...
    if (params.treeset_file) {
      ...
      if (params.topotest_replicates && info.size() > 1) {
        if (params.do_weighted_test) {
          out << "Tree      logL    deltaL  bp-RELL    p-KH     p-SH    p-WKH  "
                 "  p-WSH    c-ELW"
              << endl;
          out << "-------------------------------------------------------------"
                 "------------------"
          ...
          out << "Tree      logL    deltaL  bp-RELL    p-KH     p-SH    c-ELW"
              << endl;
          out << "-------------------------------------------------------------"
              << endl;
        }
      ...
      if (params.topotest_replicates) {
        out << "deltaL  : logL difference from the maximal logl in the set."
            << endl
        ...
        out << "c-ELW   : Expected Likelihood Weight (Strimmer & Rambaut 2002)."
            << endl
            << endl
            << "Plus signs denote the 95% confidence sets." << endl
            << "Minus signs denote significant exclusion." << endl
    ...
    out << "TIME STAMP" << endl
        ...
        << "Total CPU time used: " << (double)params.run_time << " seconds ("
        << convert_time(params.run_time) << ")" << endl
  ...
  if (params.gbo_replicates) {
    if (params.maximum_parsimony)
      cout << endl
           << "Maximum parsimony bootstrap approximation results written to:"
           << endl
  ...
  if (params.treeset_file) {
    cout << "  Evaluated user trees:          " << params.out_prefix << ".trees"
         << endl;
...
void printAnalysisInfo(int model_df, IQTree &iqtree, Params &params) {
  ...
  if (params.min_iterations > 0) {
    cout << "Tree search algorithm: "
         << (params.snni ? "Stochastic nearest neighbor interchange" : "IQPNNI")
         << endl;
    ...
    if (params.stop_condition == SC_REAL_TIME) {
      ...
      cout << "min " << params.min_correlation << " correlation coefficient"
           << endl;
    }

    if (!params.snni) {
      cout << "Number of representative leaves  : " << params.k_representative
           << endl;
    ...
    cout << "NNI assessed on: " << ((params.nni5) ? "5 branches" : "1 branch")
         << endl;
  }
  ...
  cout << "Number of Newton-Raphson steps in NNI evaluation and branch length "
          "optimization: "
...
void computeInitialDist(Params &params, IQTree &iqtree, string &dist_file) {
  ...
  if (params.compute_jc_dist || params.compute_obs_dist ||
    ...
    longest_dist = iqtree.computeDist(params, iqtree.aln, iqtree.dist_matrix,
...
void computeInitialTree(Params &params, IQTree &iqtree, string &dist_file,
  ...
  if (params.user_file) {
    ...
    if ((params.SSE == LK_EIGEN || params.SSE == LK_EIGEN_SSE) &&
      ...
      cout << "NOTE: Changing to old kernel as input tree is multifurcating"
           << endl;
      params.SSE = LK_SSE;
    }
  } else
    ...
    case STT_PLL_PARSIMONY:
      ...
      if (params.maximum_parsimony) {
        _pllComputeRandomizedStepwiseAdditionParsimonyTree(
            iqtree.pllInst, iqtree.pllPartitions, params.sprDist, &iqtree);
      } else {
...
void initializeParams(Params &params, IQTree &iqtree,
  ...
  if (params.model_name.substr(0, 4) == "TEST") {
    ...
    cout << "NOTE: MODEL SELECTION REQUIRES AT LEAST "
         << ((double)mem_size * sizeof(double) / 1024.0) / 1024 << " MB MEMORY!"
         << endl;
...
void pruneTaxa(Params &params, IQTree &iqtree, double *pattern_lh,
  ...
  if (params.aLRT_threshold <= 100 &&
    ...
    cout << num_low_support
         << " branches show low support values (<= " << params.aLRT_threshold
         << "%)" << endl;

    // tree.drawTree(cout);
    cout << "Collapsing stable clades..." << endl;
    iqtree.collapseStableClade(params.aLRT_threshold, pruned_taxa, linked_name,
                               iqtree.dist_matrix);
  ...
  if (!pruned_taxa.empty()) {
    ...
    cout << "Log-likelihood after optimizing partial tree: " << iqtree.curScore
         << endl;
  }
}
...
void printMiscInfo(Params &params, IQTree &iqtree, double *pattern_lh) {
  ...
  if (params.mvh_site_rate) {
    RateMeyerHaeseler *rate_mvh =
        new RateMeyerHaeseler(params.rate_file, &iqtree, params.rate_mh_type);
    cout << endl
  ...
  if (params.print_site_rate) {
    ...
    if (iqtree.isSuperTree()) {
      ...
      try {
        ...
        for (PhyloSuperTree::iterator it = stree->begin(); it != stree->end();
             it++, part++) {
          out << "SITE RATES FOR PARTITION " << stree->part_info[part].name
              << ":" << endl;
...
void printFinalSearchInfo(Params &params, IQTree &iqtree,
  ...
  if (iqtree.isSuperTree()) {
    ...
    cout << stree->evalNNIs << " NNIs evaluated from " << stree->totalNNIs
         << " all possible NNIs ( "
         << (int)(((stree->evalNNIs + 1.0) / (stree->totalNNIs + 1.0)) * 100.0)
         << " %)" << endl;
...
void runTreeReconstruction(Params &params, string &original_model,
  ...
  // Make sure that no partial likelihood of IQ-TREE is initialized when PLL is
  // used to save memory
  if (params.pll) {
  ...
  if (params.min_iterations > 0) {
    ...
    if (!params.user_file && (params.start_tree == STT_PARSIMONY ||
      ...
      // Because the following doNNISearch does not have ratchet alignment setup
      // yet, we have to turn-off ratchet
      ...
      //                        iqtree.pllInst->start->back, PLL_TRUE, PLL_TRUE,
      ...
      //                nni_steps); iqtree.setBestTree(iqtree.getTreeString(),
      //                iqtree.curScore);
      //            }
      //
      //        	if (iqtree.isSuperTree())
...
void runStandardBootstrap(Params &params, string &original_model,
  ...
  for (int sample = 0; sample < params.num_bootstrap_samples; sample++) {
    ...
    if (alignment->isSuperAlignment()) {
      if (params.partition_type) {
        boot_tree = new PhyloSuperTreePlen(
            (SuperAlignment *)bootstrap_alignment, (PhyloSuperTree *)tree);
      } else {
    ...
    if (params.num_bootstrap_samples == 1)
      reportPhyloAnalysis(params, original_model, *bootstrap_alignment,
                          *boot_tree, model_info, removed_seqs, twin_seqs);
  ...
  if (params.print_bootaln)
    cout << "  Bootstrap alignments:     " << params.out_prefix << ".bootaln"
         << endl;
  cout << "  Bootstrap trees:          " << params.out_prefix << ".boottrees"
       << endl;
  if (params.consensus_type == CT_CONSENSUS_TREE)
    cout << "  Consensus tree:           " << params.out_prefix << ".contree"
         << endl;
  cout << endl;
}
  ...
  if (alignment->isSuperAlignment()) {
    ...
    if (params.print_subaln)
      ((SuperAlignment *)alignment)
          ->printSubAlignments(params, ((PhyloSuperTree *)iqtree)->part_info);
...
void runPhyloAnalysis(Params &params) {
  ...
  if (params.partition_file) {
    ...
    if (params.partition_type) {
      ...
      if (params.nni5)
        outError("-nni5 option is unsupported yet for proportitional partition "
                 "model. please use -nni1 option");
      if (params.aLRT_replicates)
        outError("-alrt option is unsupported yet for proportitional partition "
                 "model");
      // initialize supertree - Proportional Edges case, "-spt p" option
      tree = new PhyloSuperTreePlen(params);
    } else {
      // initialize supertree stuff if user specifies partition file with -sp
      // option
  ...
  if (params.maximum_parsimony &&
    ...
    optimizeAlignment(
        tree,
        params); // Diep: this is to rearrange columns for better speed in REPS
  }
...
void assignBootstrapSupport(const char *input_trees, int burnin, int max_count,
  ...
  if (params && detectInputFile((char *)input_trees) == IN_NEXUS) {
    ...
    i = 0;
    for (StrVector::iterator sit = sgtaxname.begin(); sit != sgtaxname.end();
         sit++, i++) {
...
void computeConsensusTree(const char *input_trees, int burnin, int max_count,
  ...
  if (params && detectInputFile((char *)input_trees) == IN_NEXUS) {
    ...
    /*		StrVector sgtaxname;
     ...
     i = 0;
     for (StrVector::iterator sit = sgtaxname.begin(); sit != sgtaxname.end();
     sit++, i++) { Node *leaf = mytree.findLeafName(*sit); if (!leaf)
     outError("Tree does not contain taxon ", *sit); leaf->id = i;
     }*/
    scale /= sg.maxWeight();
  } else {
    boot_trees.init(input_trees, rooted, burnin, max_count, tree_weight_file);
  ...
  if (params->print_splits_file) {
    ...
    cout << "Non-trivial split supports printed to star-dot file " << out_file
         << endl;
  }
}
...
void optimizeAlignment(IQTree *&tree, Params &params) {
  ...
  if (params.user_file) {
    ...
    cout << "\nComputing random stepwise addition parsimony tree for sorting "
            "patterns..."
         << endl;
    // tree->initTopologyByPLLRandomAdition(params); // this pll version needs
    // further sync to work with the rest
    tree->computeParsimonyTree(
        params.out_prefix,
        tree->aln); // this iqtree version plays nicely with the rest
    cout << "Time for random stepwise addition parsimony tree construction: "
  ...
  if (params.sort_alignment) {
    cout << "Reordering patterns in alignment by decreasing order of pattern "
            "parsimony... ";
    start = getCPUTime();
    // reordering patterns
    PatternComp pcomp;
