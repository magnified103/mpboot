  ...
  try {
    ...
    for (seq1 = 0; seq1 < nset - 1; seq1++) {
      if (areas_boundary[seq1 * nset + seq1] <= 1e-6)
        throw "Diagonal elements of distance matrix should represent the "
              "boundary of single areas";
      for (seq2 = seq1 + 1; seq2 < nset; seq2++)
        if (areas_boundary[seq1 * nset + seq2] !=
            areas_boundary[seq2 * nset + seq1])
          throw "Shared boundary between " + areas->getSet(seq1)->name +
              " and " + areas->getSet(seq2)->name + " is not symmetric";
    }
...
void parseArg(int argc, char *argv[], Params &params) {
  ...
  params.cutoff_percent = 10; // Diep: Default = 10 for UFBoot-MP release;
  ...
  for (cnt = 1; cnt < argc; cnt++) {
    try {
      ...
      if (strcmp(argv[cnt], "-pp") == 0) {
        ...
        convert_range(argv[cnt], params.min_proportion, params.pd_proportion,
                      params.step_proportion);
      ...
      if (strcmp(argv[cnt], "-starttree") == 0) {
        ...
        if (strcmp(argv[cnt], "BIONJ") == 0)
          ...
          throw "Invalid option, please use -starttree with BIONJ or PARS or "
                "PLLPARS";
        continue;
      }
      ...
      if (strcmp(argv[cnt], "-bb") == 0) {
        ...
        params.stop_condition =
            SC_UNSUCCESS_ITERATION; // Diep: because MP already has refinement
      ...
      if (strcmp(argv[cnt], "-topboot") == 0) {
        cnt++;
        if (cnt >= argc)
          throw "Use -topboot <number of top trees stored for each boot "
                "sample>";
      ...
      if (strcmp(argv[cnt], "-ratchet_percent") == 0) {
        cnt++;
        if (cnt >= argc)
          throw "Use -ratchet_percent <percentage of characters in subset "
                "picked for ratchet>";
      ...
      if (strcmp(argv[cnt], "-spr_maxtrav") == 0 ||
        ...
        params.sprDist = params.spr_maxtrav; // Diep: hopefully this speed the
      ...
      if (strcmp(argv[cnt], "-cand_cutoff") == 0) {
        cnt++;
        if (cnt >= argc)
          throw "Use -cand_cutoff <integer #s from 0 to 100 for selecting top "
      ...
      if (strcmp(argv[cnt], "-test_mode") == 0) {
        //            	if(argc != 7)
        //            		throw "These options are to compute weighted
        //            parsimony score.\nUse -s [alignment_file] -test_mode
        //            [tree_file] -cost fitch/[cost_file]";
        // if(argc != 5)
        // 	throw "These options are to convert newick tree string of taxa
      ...
      if (strcmp(argv[cnt], "-distinct_iter_top_boot") == 0) {
        cnt++;
        if (cnt >= argc)
          throw "Use -distinct_iter_top_boot <# of boot trees (before "
      ...
      if (strcmp(argv[cnt], "-snni") == 0) {
        ...
        // Minh: why do you turn this on? it doubles curPerStrength at some
        // point
        // params.adaptPert = true;
        continue;
      }
...
void usage(char *argv[], bool full_command) {
  ...
  //    cout << "Usage: " << argv[0] << " [OPTIONS] <file_name> [<output_file>]"
  ...
  //    into optimal sets" << endl; cout << "  -e <file>         File containing
  //    branch/split scale and taxa weights" << endl; cout << "  -all Identify
  //    all multiple optimal sets" << endl; cout << "  -lim <max_limit>  The
  //    maximum number of optimal sets for each k if -a is specified" << endl;
  //    cout << "  -min              Compute minimal sets (default: maximal)" <<
  //    endl; cout << "  -1out             Print taxa sets and scores to
  //    separate files" << endl; cout << "  -oldout           Print output
  //    compatible with version 0.3" << endl; cout << "  -v Verbose mode" <<
  //    endl; cout << endl; cout << "OPTIONS FOR PHYLOGENETIC DIVERSITY (PD):"
  //    << endl; cout << "  -root             Make the tree ROOTED, default is
  //    unrooted" << endl; cout << "    NOTE: this option and -o <taxon> cannot
  //    be both specified" << endl; cout << "  -g                Run greedy
  //    algorithm only (default: auto)" << endl; cout << "  -pr Run pruning
  //    algorithm only (default: auto)" << endl; cout << endl;
  //    /*
  ...
  //    "OPTIONS FOR BUDGET CONSTRAINTS:" << endl; cout << "  -u <file> File
  //    containing total budget and taxa preservation costs" << endl; cout << "
  //    -b <budget>       Total budget to conserve taxa" << endl; cout << "  -b
  //    <min>:<max>    Find all sets with budget from <min> to <max>" << endl;
  //    cout << "  -b <min>:<max>:<step>" << endl;
  //    cout << "                    Find optimal sets with budget min,
  //    min+step, min+2*step,..." << endl; cout << endl; cout << "OPTIONS FOR
  //    AREA ANALYSIS:" << endl; cout << "  -ts <taxa_file>   Compute/maximize
  //    PD/SD of areas (combine with -k to maximize)" << endl; cout << "  -excl
  //    Compute exclusive PD/SD" << endl; cout << "  -endem            Compute
  //    endemic PD/SD" << endl; cout << "  -compl <areas>    Compute
  //    complementary PD/SD given the listed <areas>" << endl; cout << endl;
  //
  //    cout << "OPTIONS FOR VIABILITY CONSTRAINTS:" << endl;
  //    cout << "  -eco <food_web>   File containing food web matrix" << endl;
  //    cout << "  -k% <n>           Find optimal set of size relative the total
  //    number of taxa" << endl; cout << "  -diet <min_diet>  Minimum diet
  //    portion (%) to be preserved for each predator" << endl; cout << endl;
  ...
  //    (normalized)" << endl; cout << "                    expected frequencies
  //    given in site_ll_file" << endl;
  //	*/
  //
  ...
  //    cout << endl;
  //    //cout << "HIDDEN OPTIONS: see the source code file pda.cpp::parseArg()"
  //    << endl;
...
void usage_iqtree(char *argv[], bool full_command) {
  ...
  cout
      ...
      << "  -st <data_type>      BIN, DNA, AA, CODON, or MORPH (default: "
         "auto-detect)"
      << endl
      << "  -sp <partition_file> Partition model specification in NEXUS format."
      << endl
      << "                       For single model use the -m option (see below)"
      << endl
      << "  -z <trees_file>      Compute log-likelihoods for all trees in the "
         "given file"
      << endl
      ...
      << endl
#ifdef _OPENMP
      << "  -omp <#cpu_cores>    Number of cores/threads to use (default: all "
         "cores)"
      << endl
#endif
      ...
      << "                  DNA: HKY (default), JC, F81, K2P, K3P, K81uf, "
         "TN/TrN, TNef,"
      << endl
      << "                       TIM, TIMef, TVM, TVMef, SYM, GTR, or 6-digit "
         "model"
      << endl
      ...
      << "              Protein: WAG (default), Poisson, cpREV, mtREV, "
         "Dayhoff, mtMAM,"
      << endl
      << "                       JTT, LG, mtART, mtZOA, VT, rtREV, DCMut, PMB, "
         "HIVb,"
      << endl
      ...
      << "      Model selection: TEST or TESTONLY to auto-select the best-fit "
         "model."
      << endl
      ...
      << "                       counted, optimized, user-defined, equal state "
         "frequency"
      << endl
      ...
      << "  -i <p_invar>         Proportion of invariable sites (default: "
         "estimate)"
      << endl
      << "  -mh                  Computing site-specific rates to .mhrate file "
         "using"
      << endl
      << "                       Meyer & von Haeseler (2003) method"
      << endl
      //<< "  -c <#categories>     Number of Gamma rate categories (default: 4)"
      //<< endl
      << endl
      << "TEST OF MODEL HOMOGENEITY:" << endl
      << "  -m WHTEST            Testing model (GTR+G) homogeneity assumption "
         "using"
      << endl
      ...
      //            << "  -k <#representative> IQP: Size of representative leaf
      //            set (default: 4)" << endl
      ...
      //            << "  -spc <level>         Confidence level for NNI adaptive
      //            search (default 0.95)" << endl
      ...
      //            << endl
      //            << "  -seed <number>       Random seed number, normally used
      //            for debugging purpose" << endl
      ...
      << endl
      << "  -sup <target_tree>   Assigning support values for <target_tree> to "
         ".suptree"
      << endl
      << endl
      << "ROBINSON-FOULDS DISTANCE:" << endl
      << "  -rf_all              Computing all-to-all RF distances of trees in "
         "<treefile>"
      << endl
      ...
      << "  -zb <#replicates>    BP,KH,SH,ELW tests with RELL for trees passed "
         "via -z"
      << endl
  ...
  cout << "                       min, mean, and max branch lengths of random "
          "trees."
       << endl;
...
void usage_mpboot(char *argv[], bool full_command) {
  ...
  cout
      ...
      << endl
      << "  -ratchet_iter <number>    Number of non-ratchet iterations before "
         "each ratchet iteration (default: 1)"
      << endl
      << "  -ratchet_wgt <number>     Weight to add to each site selected for "
         "perturbation during ratchet (default: 1)"
      << endl
      << "  -ratchet_percent <number> Percentage of informative sites selected "
      ...
      << "  -cost <file>              Read <file> for the matrix of transition "
...
int init_random(int seed) /* RAND4 */ {
...
#else /* PARALLEL */
  {
    ...
    if (verbose_mode >= VB_MED) {
      cout << "(" << PP_Myid << ") !!! random seed set to " << seed << ", " << n
           << " drawn !!!" << endl;
    }
  }
#endif
